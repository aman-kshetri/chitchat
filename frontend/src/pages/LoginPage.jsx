    ...
    <div className='flex min-h-full items-center justify-center overflow-y-auto backdrop-blur-2xl'>
      ...
      <div className='flex w-full max-w-4xl flex-col items-center gap-8 p-4 sm:p-8 md:flex-row md:gap-20'>
        ...
        <div className='flex w-full justify-center md:w-1/2'>
          <img src={assets.logo_icon} alt="" className='w-20 max-w-full sm:w-32 md:w-auto md:max-w-2xl' />
        </div>
        ...
        <form onSubmit={onSubmitHandler} className='flex flex-col gap-4 w-full'>
          <h2 className='text-2xl font-medium text-center'>
            {currState}
            {isDataSubmitted && <img onClick={()=>setIsDataSubmitted(false)} src={assets.arrow_icon} alt="" className='inline-block w-5 cursor-pointer' /> }
          </h2>
          ...
          {currState === "Sign Up" && isDataSubmitted && (
            <textarea rows={4} onChange={(e) => setBio(e.target.value)} value={bio} placeholder='Provide short bio... ' required className='p-2 border border-gray-500 rounded-md focus:outline-none' />
          )}
            ...
            <div className='flex items-center justify-center gap-2'>
              <input type="checkbox" />
              <p className='text-xs'>Agree to the terms of use and privacy policy.</p>
            </div>

            <div>
              {currState === "Sign Up" ? (
                <p>Already have an account? <span onClick={() => { setCurrState("Login"); setIsDataSubmitted(false); }} 
                className='text-[#ec0f0f] cursor-pointer'>Login here</span></p>
              ) : (
                <p>Create an account <span onClick={() => setCurrState("Sign Up")} className='text-[#ec0f0f] cursor-pointer'>Click here</span></p>
              )}
            </div>
